    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            if "error" not in result:
                ...
                error_message = result['error'].get('message', 'Unknown error from Firebase')
                current_app.logger.error(f"[LOGIN] Firebase error: {error_message}")
                ...
                if error_message in ["EMAIL_NOT_FOUND", "INVALID_PASSWORD"]:
                    ...
                    flash(f"An authentication error occurred: {error_message}. Please try again.", "danger")

        except requests.exceptions.RequestException as e:
            current_app.logger.exception(f"[LOGIN] Firebase request error: {e}")
