    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            try:
                ...
                if local_user and local_user.role.value.lower() == required_role.lower():
                    session['user_role'] = local_user.role.value
                    return f(*args, **kwargs)
            except ValueError:
                current_app.logger.error(
                    f"[ROLE FALLBACK - SQLAlchemy] Invalid user_id format: {user_id}"
                )
            except Exception as e:
                current_app.logger.error(
                    f"[ROLE FALLBACK - SQLAlchemy] Error checking local DB role for user {user_id}: {e}",
                    exc_info=True,
                )
            ...
            try:
                ...
                if user_doc.exists:
                    ...
                    if firebase_role and firebase_role.lower() == required_role.lower():
                        session['user_role'] = firebase_role
                        return f(*args, **kwargs)
            except Exception as e:
                current_app.logger.error(
                    f"[ROLE FALLBACK - Firebase] Error checking Firebase role for user {user_id}: {e}",
                    exc_info=True,
                )
